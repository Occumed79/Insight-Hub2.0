...
function mergeCandidates(candidates: CompanyLocationCandidate[]): CompanyLocationCandidate[] {
  ...
  return Array.from(merged.values()).slice(0, 260);
}
...
function buildResearchSources(companyName: string, officialWebsite?: string) {
...
router.post("/locations/discover", async (req, res) => {
  ...
  try {
    ...
    res.status(200).json({
      ...
      researchSources: buildResearchSources(baseline.canonicalName, baseline.officialWebsite),
    ...
    console.error("AI location discovery error:", error);
    res.status(500).json({
      ok: false,
      entityId: entity.id,
      companySaved: true,
