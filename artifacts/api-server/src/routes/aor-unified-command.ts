...
const SCOPES: Record<CommandId, Scope> = {
  northcom: { label: "USNORTHCOM", terms: ["United States", "USA", "Alaska", "Canada", "Mexico", "Greenland", "Bahamas", "Puerto Rico", "California"], bbox: [-170, 23, -50, 85] },
...
function excluded(scope: Scope, haystack: string) { return (scope.exclude ?? []).some((term) => hasTerm(haystack, term)); }
function scopeMatch(scope: Scope, haystack: string) { return !excluded(scope, haystack) && scope.terms.some((term) => hasTerm(haystack, term)); }
...
function dateString(value: unknown) { const raw = text(value); if (!raw) return ""; const parsed = new Date(raw); return Number.isNaN(parsed.valueOf()) ? raw : parsed.toISOString(); }
...
async function whoFeed() {
  // The live WHO endpoint accepts this minimal query. The removed $select
  // projection is what produced the HTTP 400 visible in the deployed AOR page.
  const params = new URLSearchParams({ "$orderby": "PublicationDateAndTime desc", "$top": "250" });
  return record(await fetchJson(`https://www.who.int/api/news/diseaseoutbreaknews?${params}`));
}
function whoItem(raw: unknown) {
  const item = record(raw);
  const title = text(item.OverrideTitle) || text(item.Title) || "WHO Disease Outbreak News";
  const summary = stripHtml(text(item.Summary) || text(item.Overview) || text(item.Assessment) || text(item.Response)).slice(0, 900);
  const donId = text(item.DonId);
  const path = text(item.ItemDefaultUrl);
  return {
    id: donId || text(item.Id) || title,
    title,
    publicationDate: dateString(item.PublicationDateAndTime || item.PublicationDate),
    summary,
    ...
    searchable: `${title} ${summary} ${text(item.Assessment)} ${text(item.Advice)} ${text(item.Response)}`,
  };
}
async function whoForScope(scope: Scope) {
  const payload = await whoFeed();
  return array(payload.value).map(whoItem).flatMap((item) => {
    const matchedArea = scope.terms.find((term) => hasTerm(item.searchable, term)) ?? "";
    return matchedArea && !excluded(scope, item.searchable) ? [{ id: item.id, title: item.title, publishedAt: item.publicationDate, summary: item.summary, matchedArea, url: item.sourceUrl }] : [];
  }).slice(0, 12);
}
async function gdacsForScope(scope: Scope) {
  ...
  return payloadArray(payload).flatMap((raw) => {
    const feature = record(raw); const properties = record(feature.properties); const props = Object.keys(properties).length ? properties : feature; const geometry = record(feature.geometry); const coord = firstCoordinate(geometry.coordinates);
    const longitude = coord?.[0] ?? numberValue(props.longitude ?? props.lon ?? props.lng); const latitude = coord?.[1] ?? numberValue(props.latitude ?? props.lat);
    const eventType = text(props.eventtype) || text(props.eventType) || text(props.type) || "Disaster"; const eventId = text(props.eventid) || text(props.eventId) || text(props.id) || text(feature.id);
    const title = text(props.name) || text(props.title) || text(props.eventname) || text(props.description) || `${eventType} event`; const country = text(props.country) || text(props.countryname) || text(props.countryName) || text(props.country_name) || text(props.iso3); const searchable = `${title} ${country} ${text(props.description)} ${text(props.location)}`;
    if (excluded(scope, searchable) || (!scopeMatch(scope, searchable) && !pointInScope(scope, longitude, latitude))) return [];
    const id = `${eventType}-${eventId || title}`; if (seen.has(id)) return []; seen.add(id);
    return [{ id, title, eventType, country, alertLevel: text(props.alertlevel) || text(props.alertLevel) || text(props.alert), fromDate: dateString(props.fromdate ?? props.fromDate ?? props.date ?? props.startdate), toDate: dateString(props.todate ?? props.toDate ?? props.enddate), latitude, longitude, url: text(props.url) || text(props.link) || text(props.weburl) || (eventId ? `https://www.gdacs.org/resources.aspx?eventid=${encodeURIComponent(eventId)}&eventtype=${encodeURIComponent(eventType)}` : "https://www.gdacs.org/") }];
...
async function usgsForScope(scope: Scope) {
  ...
  return array(payload.features).flatMap((raw) => {
    ...
    if (excluded(scope, searchable) || (!scopeMatch(scope, searchable) && !pointInScope(scope, longitude, latitude))) return [];
    return [{ id: text(feature.id) || title, title, place, magnitude: numberValue(props.mag), occurredAt: Number.isFinite(Number(props.time)) ? new Date(Number(props.time)).toISOString() : "", url: text(props.url) || "https://earthquake.usgs.gov/earthquakes/map/", tsunami: Number(props.tsunami) === 1, latitude, longitude, depthKm: Array.isArray(geometry.coordinates) ? numberValue(geometry.coordinates[2]) : null }];
  }).slice(0, 12);
}
...
async function handleCountryWho(req: Request, res: Response) {
  res.setHeader("Cache-Control", "no-store");
  const country = text(req.query.country).slice(0, 100);
  if (!country) return res.status(400).json({ ok: false, error: "country is required" });
  try {
    const payload = await whoFeed();
    const items = array(payload.value).map(whoItem);
    const needle = normalize(country);
    const matched = items.filter((item) => normalize(item.searchable).includes(needle));
    const selected = (matched.length ? matched : items.slice(0, 12)).slice(0, 20);
    ...
    return res.json({
      ...
      outbreaks: selected.map(({ searchable: _searchable, ...item }) => ({ ...item, matchedCountry: matchedIds.has(item.id) })),
    });
  } catch (error) {
    return res.status(502).json({ ok: false, error: error instanceof Error ? error.message : "WHO Disease Outbreak News unavailable." });
  }
}

// Registered before the legacy AOR/reviewer routers so these corrected paths
// own both command and country WHO requests used by the unified map workspace.
router.get("/aor/unified-command", handleUnifiedCommand);
router.get("/reviewer-tools/aor", handleUnifiedCommand);
router.get("/aor/health-outbreaks", handleCountryWho);
