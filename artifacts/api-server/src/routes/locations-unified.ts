...
router.post("/locations/discover", async (req, res) => {
  ...
  try {
    ...
    res.status(200).json({
      ...
      source: "Official website + Foursquare Places + TomTom Places + Geoapify + LocationIQ + Keenable/TinyFish Search+Fetch/LangSearch/Exa/Tavily + Groq/Gemini/Cerebras + Wikidata + OpenStreetMap + Photon",
      sourceDiagnostics: diagnostics,
      coverage: {
