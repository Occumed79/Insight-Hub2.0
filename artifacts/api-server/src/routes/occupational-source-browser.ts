...
type BlsDimensionSpec = {
  label: string;
  file: string;
};

const BLS_DATASETS: Record<BlsDatasetKey, {
  title: string;
  description: string;
  officialUrl: string;
  dimensions: Record<string, BlsDimensionSpec>;
}> = {
  is: {
    ...
    dimensions: {
      ...
      case_type: { label: "Case types", file: "is.case_type" },
      data_type: { label: "Data types / measures", file: "is.data_type" },
...
async function fetchOnetPath(path: string): Promise<unknown> {
  ...
  return fetchJson(`${ONET_BASE}${path}`, {
    headers: { "X-API-Key": key, Accept: "application/json", "User-Agent": "Occu-Med-Insight-Hub/2.0 complete O*NET browser" },
  });
}
...
router.get("/occupational-source-browser/onet/occupations", async (req: Request, res: Response) => {
  ...
  try {
    ...
    return res.status(502).json({ ok: false, error: error instanceof Error ? error.message.slice(0, 400) : "O*NET occupation catalog failed." });
  }
});

router.get("/occupational-source-browser/onet/content-model/:family", async (req: Request, res: Response) => {
  const family = ONET_DATA_FAMILIES[req.params.family];
  ...
  try {
    ...
    return res.json({ ok: true, family: req.params.family, label: family.label, source: "O*NET Web Services API v2", payload });
  } catch (error) {
    return res.status(502).json({ ok: false, error: error instanceof Error ? error.message.slice(0, 400) : "O*NET content model failed." });
  }
});
...
router.get("/occupational-source-browser/onet/occupation/:code", async (req: Request, res: Response) => {
  ...
  try {
    ...
    return res.status(502).json({ ok: false, error: error instanceof Error ? error.message.slice(0, 400) : "O*NET occupation overview failed." });
  }
});
...
router.get("/occupational-source-browser/onet/occupation/:code/content", async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (target.origin !== ONET_BASE || !decodeURIComponent(target.pathname).startsWith(prefix)) {
      return res.status(400).json({ ok: false, error: "Content URL must be an O*NET URL listed for the selected occupation." });
    }
    ...
    const payload = await fetchJson(target.toString(), {
      headers: { "X-API-Key": getOnetKey() as string, Accept: "application/json", "User-Agent": "Occu-Med-Insight-Hub/2.0 complete O*NET browser" },
    });
    return res.json({ ok: true, source: "O*NET Web Services API v2", url: target.toString(), payload });
  } catch (error) {
    return res.status(502).json({ ok: false, error: error instanceof Error ? error.message.slice(0, 400) : "O*NET content request failed." });
  }
});
...
router.get("/occupational-source-browser/bls/catalog", async (req: Request, res: Response) => {
  ...
  try {
    const catalog = await loadBlsCatalog(dataset);
    return res.json({ ok: true, source: "BLS public time-series mapping files", catalog });
  } catch (error) {
    return res.status(502).json({ ok: false, error: error instanceof Error ? error.message.slice(0, 400) : "BLS catalog failed." });
  }
});
...
async function fetchBlsSeries(seriesId: string, startYear: number, endYear: number): Promise<unknown> {
  ...
  return fetchJson(BLS_API, {
    method: "POST",
    headers: { "Content-Type": "application/json", Accept: "application/json", "User-Agent": "Occu-Med-Insight-Hub/2.0 BLS browser" },
...
router.get("/occupational-source-browser/bls/is-series", async (req: Request, res: Response) => {
  ...
  if (![supersector.length === 3, industry.length === 6, dataType.length === 1, caseType.length === 1, area.length === 3].every(Boolean)) {
    return res.status(400).json({ ok: false, error: "Complete BLS IS dimension codes are required." });
  }
  ...
  try {
    const payload = await fetchBlsSeries(seriesId, startYear, endYear);
    return res.json({ ok: true, seriesId, startYear, endYear, source: "BLS Public Data API v2", payload });
  } catch (error) {
    return res.status(502).json({ ok: false, seriesId, error: error instanceof Error ? error.message.slice(0, 400) : "BLS IS series request failed." });
  }
});
...
router.get("/occupational-source-browser/bls/fa-series", async (req: Request, res: Response) => {
  ...
  const area = clean(req.query.area, 3).toUpperCase();
  if (![category.length === 3, detail.length === 6, datatype.length === 1, caseCode.length === 1, area.length === 3].every(Boolean)) {
    return res.status(400).json({ ok: false, error: "Complete BLS CFOI dimension codes are required." });
  }
  ...
  try {
    ...
    return res.status(502).json({ ok: false, seriesId, error: error instanceof Error ? error.message.slice(0, 400) : "BLS CFOI series request failed." });
  }
});
  ...
  try {
    ...
    const tables = await Promise.all(Object.entries(OSHA_TABLES).map(async ([id, spec]) => {
      const [columns, count] = await Promise.all([
        pool.query<{ column_name: string; data_type: string; ordinal_position: number }>(`
          SELECT column_name, data_type, ordinal_position::int
    ...
    return res.status(500).json({ ok: false, error: error instanceof Error ? error.message.slice(0, 400) : "OSHA database catalog failed." });
  }
});
...
router.get("/occupational-source-browser/osha/rows", async (req: Request, res: Response) => {
  ...
  try {
    ...
    return res.status(500).json({ ok: false, error: error instanceof Error ? error.message.slice(0, 400) : "OSHA row browser failed." });
  }
});
...
router.get("/occupational-source-browser/datagov/catalog", async (req: Request, res: Response) => {
  ...
  try {
    ...
    return res.status(502).json({ ok: false, error: error instanceof Error ? error.message.slice(0, 400) : "Data.gov catalog browser failed." });
  }
});
...
router.get("/occupational-source-browser/datagov/dataset/:id", async (req: Request, res: Response) => {
  ...
  try {
    ...
    return res.status(502).json({ ok: false, error: error instanceof Error ? error.message.slice(0, 400) : "Data.gov dataset metadata failed." });
  }
});

router.get("/occupational-source-browser/manifest", (_req: Request, res: Response) => {
  return res.json({
    ok: true,
