...
export async function resolveExternalGeospatial(request: GeospatialResolveRequest, deps: ResolverDependencies): Promise<GeospatialResolution> {
  ...
  if (primary) {
    ...
    if (lowGeocodio || weakSite) {
      ...
      if (secondary && haversineKm(primary, secondary) <= 50) {
        const corroborated = resolutionFromCandidate(request, primary, true);
        if (corroborated.status === "resolved") return corroborated;
      }
    } else {
