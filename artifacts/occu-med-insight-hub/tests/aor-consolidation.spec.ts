...
test("AOR Factors is an immersive map with one floating Apple-glass sidebar", async ({ page }) => {
  ...
  await expect(page.getByTestId("aor-orb-overlay")).toBeVisible();
  await page.evaluate(() => window.dispatchEvent(new CustomEvent("aor:projection-change", { detail: { mode: "2d", projection: "mercator" } })));
  await expect(page.getByTestId("aor-orb-overlay")).toHaveCount(0);
  await page.evaluate(() => window.dispatchEvent(new CustomEvent("aor:projection-change", { detail: { mode: "3d", projection: "globe" } })));
  await expect(page.getByTestId("aor-orb-overlay")).toBeVisible();
