...
function StateImpact({ states, perCapita, conflicts }: { states: WarCostsRow[]; perCapita: WarCostsRow[]; conflicts: WarCostsRow[] }) {
  ...
  const post911Deaths = conflicts.filter((row) => wcNumber(row, "startYear") >= 2001 || (wcNumber(row, "endYear") || 0) >= 2001).reduce((sum, row) => sum + wcConflictDeaths(row), 0);
  const stateCasualtyShare = share > 0 ? post911Deaths * share : 0;
  ...
  return <GlassCard className="p-5"><h3 className="text-lg font-black">State Impact Calculator</h3><p className="mt-1 text-xs text-cyan-100/42">State share of roughly $8T in post-9/11 war spending, source-derived casualty context, and what the same money could have funded locally.</p><select value={key} onChange={(e) => setKey(e.target.value)} className="mt-4 min-h-11 w-full rounded-xl border border-white/10 bg-[#07101c] px-3">{states.map((row) => { const value = wcText(row, "code", "state", "slug"); return <option key={value} value={value}>{wcText(row, "state", "name")} {wcText(row, "code")}</option>; })}</select><div className="mt-4 grid gap-3 sm:grid-cols-2 xl:grid-cols-5"><Metric label="State war-cost share" value={stateWarCost ? wcMoney(stateWarCost) : "Source share unavailable"} note={explicitSharePct > 0 ? "source tax/share factor" : population > 0 ? "neutral population share" : "no fabricated allocation"} /><Metric label="Casualty equivalent" value={share > 0 ? wcInteger(stateCasualtyShare) : "—"} note={`${wcInteger(post911Deaths)} mirrored post-9/11 US deaths × state share`} /><Metric label="Current DoD spending" value={normalizedDefenseSpending ? wcMoney(normalizedDefenseSpending) : "—"} note="state footprint source" /><Metric label="Defense-linked jobs" value={wcNumber(selected, "jobs", "directJobs") ? wcInteger(wcNumber(selected, "jobs", "directJobs")) : "—"} note="source state profile" /><Metric label="Installations" value={wcInteger(wcNumber(selected, "bases", "total", "activeBases"))} note="source military footprint" /></div>{stateWarCost > 0 ? <div className="mt-4 grid gap-2 md:grid-cols-2 xl:grid-cols-4">{alternatives.map(([label, unit]) => <div key={label} className="rounded-xl border border-cyan-100/10 bg-cyan-300/[.04] p-3"><p className="text-xl font-black">{wcInteger(stateWarCost / unit)}</p><p className="mt-1 text-[9px] text-cyan-100/38">{label}</p></div>)}</div> : <div className="mt-4 rounded-xl border border-amber-200/12 bg-amber-300/[.04] p-4 text-xs text-amber-50/65">The mirrored state feeds do not currently expose a defensible allocation factor for {name}. The app intentionally does not manufacture one.</div>}</GlassCard>;
}
...
function CasualtyScale({ conflicts }: { conflicts: WarCostsRow[] }) {
  ...
  return <GlassCard className="p-5"><h3 className="text-lg font-black">Casualty Scale Calculator</h3><p className="mt-1 text-xs text-cyan-100/42">Choose a conflict, enter a city name and population, or use the same city-preset interaction exposed by WarCosts.</p><div className="mt-4 grid gap-3 md:grid-cols-3"><select value={warId} onChange={(e) => setWarId(e.target.value)} className="min-h-11 rounded-xl border border-white/10 bg-[#07101c] px-3">{conflicts.map((row) => <option key={wcConflictId(row)} value={wcConflictId(row)}>{wcConflictName(row)}</option>)}</select><input value={city} onChange={(e) => setCity(e.target.value)} className="min-h-11 rounded-xl border border-white/10 bg-black/20 px-3" placeholder="City name" /><input type="number" min={1} value={population} onChange={(e) => setPopulation(Number(e.target.value))} className="min-h-11 rounded-xl border border-white/10 bg-black/20 px-3" placeholder="Population" /></div><div className="mt-3 flex flex-wrap gap-2">{CITY_PRESETS.map(([name, size]) => <button key={name} type="button" onClick={() => choosePreset(name, size)} className="rounded-full border border-white/9 bg-black/10 px-3 py-2 text-[9px] font-bold text-cyan-50/65 hover:border-cyan-200/22">{name}</button>)}</div><div className="mt-4 grid gap-3 sm:grid-cols-2"><Metric label="US-death equivalent" value={wcInteger(wcConflictDeaths(selected) * scale)} note={`${city || "Your city"} · population ${wcInteger(population)}`} /><Metric label="Civilian-death equivalent" value={wcInteger(wcCivilianDeaths(selected) * scale)} note="same population scaling" /></div><div className="mt-4 rounded-xl border border-white/8 bg-black/10 p-4"><p className="text-xs font-bold">Source context</p><p className="mt-2 text-[10px] leading-5 text-cyan-100/42">{wcConflictName(selected)} records {wcInteger(wcConflictDeaths(selected))} US deaths and {wcInteger(wcCivilianDeaths(selected))} civilian deaths in the mirrored WarCosts conflict data.</p></div></GlassCard>;
}
