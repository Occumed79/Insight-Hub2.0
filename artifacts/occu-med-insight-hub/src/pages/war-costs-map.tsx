...
export default function WarCostsMap() {
  ...
  async function load(force = false) {
    ...
    try {
      ...
      setResponses(next);
      if (!next["conflicts.json"] || !next["base-countries.json"]) setError("Some WarCosts map feeds are unavailable; the map will render the layers that loaded successfully.");
    } catch (loadError) {
    ...
    <main className="aurora-bg min-h-screen text-white">
      ...
      <section className="relative z-10 px-5 py-8 lg:ml-[210px] lg:px-8">
        <div className="flex flex-wrap items-start justify-between gap-4">
          <HeaderBar eyebrow="WarCosts Intelligence" title="War Map" subtitle="A completely independent ArcGIS map workspace for WarCosts operational geography. It does not use the AOR MapTiler map, AOR map state, or AOR layers." />
        ...
        <div className="mt-5">
          {loading ? <GlassCard className="grid min-h-[720px] place-items-center"><div className="text-center"><Loader2 className="mx-auto h-9 w-9 animate-spin text-cyan-200" /><p className="mt-3 text-sm font-bold">Loading the independent WarCosts map feeds…</p></div></GlassCard> : <WarCostsArcGisMap conflicts={wcRows(data["conflicts.json"])} baseCountries={wcRows(data["base-countries.json"])} deployments={wcRows(data["overseas-presence.json"])} operations={wcRows(data["operations.json"])} strikes={wcRows(data["drone-strikes.json"])} />}
        </div>
      </section>
    </main>
  );
