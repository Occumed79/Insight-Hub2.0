  ...
  return (
    <main className="aurora-bg min-h-screen text-white">
      ...
      <section className="relative z-10 px-5 py-8 lg:ml-[210px] lg:px-12">
        ...
        {result?.error ? <GlassCard className="mt-5 border border-amber-200/20 p-5"><div className="flex items-center gap-3 text-amber-100"><AlertTriangle size={18} /><p className="font-semibold">Discovery failed</p></div><p className="mt-2 text-sm text-cyan-100/60">{result.error}</p></GlassCard> : null}
        ...
        {result?.ok ? (
          <>
            ...
            <GlassCard className="mt-5 border border-amber-200/20 p-5"><p className="text-xs uppercase tracking-[0.24em] text-amber-100/70">Verification rule</p><p className="mt-2 text-sm leading-6 text-cyan-100/62">{result.warning}</p></GlassCard>
            <div className="mt-5 grid gap-4 xl:grid-cols-2">
              {locations.map((location) => (
                <GlassCard key={location.id} className="p-5">
                  <div className="flex items-start justify-between gap-3"><div><p className="text-xs uppercase tracking-[0.22em] text-cyan-100/40">{location.country}</p><h3 className="mt-2 text-xl font-black text-white">{location.placeName}</h3></div><span className={`rounded-full border px-3 py-1 text-xs ${confidenceClass(location.geocodeConfidence)}`}>{location.geocodeConfidence}</span></div>
                  <p className="mt-3 text-sm leading-6 text-cyan-100/62">{location.formattedAddress}</p>
                  <div className="mt-4 grid gap-2 text-xs text-cyan-100/55 md:grid-cols-2"><p><span className="text-cyan-100/35">Source:</span> {location.geocodeSource}</p><p><span className="text-cyan-100/35">OSM:</span> {location.sourceClass}/{location.sourceType}</p><p><span className="text-cyan-100/35">Coordinates:</span> {location.coordinates[1].toFixed(5)}, {location.coordinates[0].toFixed(5)}</p><p><span className="text-cyan-100/35">Review:</span> {location.reviewStatus}</p></div>
                  {location.geocodeConfidence !== "unknown" ? <a href={mapLink(location)} target="_blank" rel="noreferrer" className="mt-4 inline-flex items-center gap-2 rounded-full border border-cyan-100/15 bg-cyan-100/5 px-4 py-2 text-xs font-semibold text-cyan-50 hover:bg-cyan-200/[0.08]"><ExternalLink size={12} />Open map candidate</a> : null}
                </GlassCard>
              ))}
            </div>
          </>
