...
export default function ReviewerAorFactorsV2Page() {
  ...
  useEffect(() => {
    ...
    async function initMap() {
      try {
        ...
        const style = sdk.MapStyle?.STREETS || sdk.MapStyle?.STREETS?.DEFAULT;
  ...
  const countrySourceStatus = (source: SourceResult | undefined, idleNote = "Select a country") => {
    if (!selectedCountry) return { status: "loading" as const, note: idleNote };
    if (source?.loading) return { status: "loading" as const, note: "Loading" };
    if (source?.error) return { status: "warn" as const, note: "Source temporarily unavailable" };
    if (source?.data?.available === false) return { status: "warn" as const, note: source.data?.sourceNotice || "Source temporarily unavailable" };
    return { status: "ok" as const, note: "Country feed loaded" };
  };
  ...
  return (
    <main className="aurora-bg min-h-screen pb-16 text-white">
      ...
      <section className="relative z-10 px-4 py-6 pt-24 lg:ml-[210px] lg:px-8 lg:pt-6 xl:px-10">
        <HeaderBar eyebrow="Operational / Environmental Intelligence" title="AOR Factors" subtitle="Choose Country mode for country-level travel and health intelligence, or AOR mode for command-wide operational feeds. Both use the same MapTiler Streets vector-tile map." />
        ...
        <Surface className="overflow-hidden">
          ...
          <div className="mt-5 grid gap-5 xl:grid-cols-[minmax(0,1.42fr)_minmax(340px,.58fr)]">
            <div className="min-w-0 space-y-4">
              <div className="overflow-hidden rounded-[24px] border border-white/13 bg-[#020812]/76 shadow-[0_22px_65px_rgba(0,0,0,.34)]">
                ...
                <div className="relative h-[650px] overflow-hidden bg-[#020812]" data-testid="aor-map-shell">
                  <div className="pointer-events-none absolute inset-0 z-[1] bg-[radial-gradient(circle_at_18%_15%,rgba(34,211,238,.035),transparent_28%),radial-gradient(circle_at_82%_76%,rgba(124,58,237,.035),transparent_30%)]" />
                  <div ref={mapHostRef} className="aor-map-tiler-host absolute inset-0" aria-label="Interactive MapTiler AOR intelligence map" />
                  {mapStatus !== "ready" ? <div className="absolute inset-0 z-20 grid place-items-center bg-[#020812]/82 p-6 text-center backdrop-blur-sm">{mapStatus === "loading" ? <div><Loader2 className="mx-auto animate-spin text-cyan-200/70" size={24} /><p className="mt-3 text-xs text-cyan-100/50">Rendering MapTiler Streets vector tiles…</p></div> : <div><AlertTriangle className="mx-auto text-amber-200/70" size={24} /><p className="mt-3 text-sm font-black text-amber-50/82">Map rendering failed</p><p className="mt-2 max-w-lg text-[10px] leading-5 text-amber-100/52">{mapError}</p></div>}</div> : null}
              ...
              {mapMode === "country" ? (
                <div className="rounded-[24px] border border-white/13 bg-gradient-to-br from-cyan-300/[0.055] via-[#04101c]/72 to-violet-300/[0.055] p-4 shadow-[0_18px_50px_rgba(0,0,0,.24)] backdrop-blur-2xl">
                  ...
                  {!selectedCountry ? <div className="mt-4 rounded-2xl border border-white/8 bg-white/[0.02] p-5 text-center text-xs text-cyan-100/42">Select a country to load its CDC travel-health profile.</div> : countrySources.health.loading ? <div className="mt-4 flex min-h-28 items-center justify-center gap-2 text-xs text-cyan-100/45"><Loader2 size={15} className="animate-spin" />Loading CDC destination guidance…</div> : countrySources.health.error || travelHealth?.available === false ? <div className="mt-4 rounded-2xl border border-amber-200/12 bg-amber-300/[0.035] p-4"><p className="text-[10px] leading-5 text-amber-50/70">{travelHealth?.sourceNotice || "CDC destination guidance is temporarily unavailable for this country. Other country intelligence remains active."}</p>{externalUrl(travelHealth?.sourceUrl) ? <a href={externalUrl(travelHealth.sourceUrl)} target="_blank" rel="noreferrer" className="mt-3 inline-flex items-center gap-1 text-[9px] font-bold text-amber-100/70">Open CDC Travelers' Health<ArrowUpRight size={10} /></a> : null}</div> : (
            ...
            <aside className="max-h-[1180px] overflow-y-auto overscroll-contain rounded-[24px] border border-white/13 bg-[#03101b]/64 p-4 shadow-[0_20px_60px_rgba(0,0,0,.28)] backdrop-blur-2xl" aria-label="Map-linked intelligence inspector">
              ...
              <InspectorSection title="International Crisis Group CrisisWatch" icon={<RadioTower size={14} className="text-violet-200/64" />}>{!selectedCountry ? <EmptyIntel>{mapMode === "country" ? "Select a country to load CrisisWatch context." : "CrisisWatch context is country-specific; switch to Country mode."}</EmptyIntel> : countrySources.crisiswatch.loading ? <EmptyIntel>Loading CrisisWatch…</EmptyIntel> : countrySources.crisiswatch.error ? <EmptyIntel>CrisisWatch is temporarily unavailable for this destination.</EmptyIntel> : crisisUpdates.length ? <div className="space-y-2">{crisisUpdates.map((item: any, index: number) => <IntelItem key={item.id || index} title={item.title || item.headline || "CrisisWatch update"} meta={formatDate(item.publishedAt || item.date)} summary={item.summary || item.description} href={item.url || item.sourceUrl} />)}</div> : <EmptyIntel>No readable CrisisWatch item matched this country.</EmptyIntel>}</InspectorSection>
