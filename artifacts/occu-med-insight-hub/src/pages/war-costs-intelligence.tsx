        ...
        <div className="flex flex-wrap items-start justify-between gap-3">
          <div className="min-w-0">
            <div className="flex flex-wrap items-center gap-2">
              <h3 className="text-base font-black text-white">{status.name}</h3>
              <span className={`rounded-full border px-2 py-1 text-[9px] font-bold uppercase tracking-wide ${refreshTone(status.refreshClass)}`}>{status.refreshClass}</span>
              {status.ok ? <CheckCircle2 size={14} className="text-emerald-300/70" /> : <AlertTriangle size={14} className="text-rose-300/75" />}
            </div>
            <p className="mt-2 max-w-4xl text-xs leading-5 text-cyan-100/46">{DATASET_EXPLANATIONS[status.name] ?? "Additional dataset discovered from the live WarCosts downloads manifest."}</p>
          </div>
          <div className="text-right"><p className="text-lg font-black text-white">{status.count.toLocaleString()}</p><p className="text-[10px] text-cyan-100/35">source records</p></div>
        </div>
        <div className="mt-3 flex flex-wrap items-center gap-2 text-[10px] text-cyan-100/34"><span>{status.source === "database" ? "Neon fallback" : status.source === "live" ? "Live source" : "Source pending"}</span><span>·</span><span>{formatDate(status.fetchedAt)}</span></div>
      </div>
      <div className="px-5 py-4">
        {loading && !response ? (
          <div className="flex min-h-24 items-center justify-center gap-2 text-xs text-cyan-100/45"><Loader2 size={16} className="animate-spin" /> Loading the complete feed…</div>
        ) : !status.ok && !response ? (
          <div className="rounded-xl border border-rose-300/15 bg-rose-300/[0.06] p-4"><p className="text-xs font-bold text-rose-100">This source did not mirror successfully.</p><p className="mt-1 text-[11px] text-rose-100/55">{status.error || "No source response was retained."}</p><button type="button" onClick={onRetry} className="mt-3 rounded-lg border border-rose-200/20 px-3 py-2 text-[10px] font-bold text-rose-50">Retry feed</button></div>
        ) : rows.length === 0 ? (
          <div className="rounded-xl border border-dashed border-cyan-100/12 p-6 text-center text-xs text-cyan-100/38">The source returned no records.</div>
        ) : (
          <>
            <div className="space-y-2">
              {visible.map((row, index) => (
                <details key={index} className="rounded-xl border border-cyan-100/8 bg-black/10 px-3 py-2.5">
                  <summary className="cursor-pointer list-none"><div className="flex items-center justify-between gap-3"><div className="min-w-0"><p className="truncate text-xs font-bold text-cyan-50">{labelForRecord(row)}</p><p className="mt-1 truncate text-[10px] text-cyan-100/35">{subtitleForRecord(row) || `Record ${index + 1}`}</p></div><ChevronRight size={14} className="shrink-0 text-cyan-100/25" /></div></summary>
                  <pre className="mt-3 max-h-[520px] overflow-auto whitespace-pre-wrap break-words rounded-lg border border-cyan-100/8 bg-black/25 p-3 text-[10px] leading-5 text-cyan-50/68">{JSON.stringify(row, null, 2)}</pre>
                </details>
              ))}
            </div>
            {rows.length > 60 && <button type="button" onClick={() => setShowAll((value) => !value)} className="mt-4 min-h-10 rounded-xl border border-cyan-200/14 bg-cyan-300/[0.06] px-4 text-[11px] font-bold text-cyan-50 transition hover:bg-cyan-300/10">{showAll ? "Collapse records" : `Show all ${rows.length.toLocaleString()} records`}</button>}
          </>
        )}
      </div>
    </GlassCard>
...
function ContractorWorkspace({ contractors }: { contractors: WarCostsContractor[] }) {
  ...
  useEffect(() => { if (!selected && contractors.length) setSelected(contractors[0]); }, [contractors, selected]);
...
export default function WarCostsIntelligence() {
  ...
  async function load(force = false): Promise<void> {
    ...
    setError("");
    try {
      const [nextOverview, contractorIntel] = await Promise.all([getWarCostsOverview(force), getWarCostsContractorIntelligence()]);
  ...
  const complete = Boolean(overview && overview.summary.failedDatasets === 0 && overview.summary.mirroredDatasets === overview.summary.discoveredDatasets);
