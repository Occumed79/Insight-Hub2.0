...
type LegalClass = { employee: boolean; injury: boolean; workersComp: boolean; occupationalHealth: boolean; occupationalDisease: boolean; disability: boolean; medicalExam: boolean; workplaceSafety: boolean; dba: boolean; tort: boolean; role: "plaintiff-name-position" | "defendant-name-position" | "unclear" };
...
function classifyLegal(reference: LegalReference, entity: string): LegalClass {
  ...
  const occupationalDisease = /\b(occupational disease|work-related illness|asbestos|silicosis|toxic exposure)\b/.test(body);
  const disability = /\b(disability|reasonable accommodation|americans with disabilities|\bada\b|interactive process)\b/.test(body);
  const medicalExam = /\b(medical exam|medical examination|fitness for duty|fitness-for-duty|medical evaluation)\b/.test(body);
  const workplaceSafety = /\b(osha|workplace safety|safety violation|occupational safety)\b/.test(body);
  const dba = /\b(defense base act|longshore|overseas contractor|\bdba\b)\b/.test(body);
  ...
  return { employee, injury, workersComp, occupationalHealth, occupationalDisease, disability, medicalExam, workplaceSafety, dba, tort, role };
}
...
export function EntityLegalReferencesPage() {
  ...
  const filtered = useMemo(() => classified.filter(({ reference, classification }) => {
    if (!(classification.employee || classification.injury || classification.workersComp || classification.occupationalHealth || classification.occupationalDisease || classification.disability || classification.medicalExam || classification.workplaceSafety || classification.dba || classification.tort)) return false;
  ...
  return <ToolShell eyebrow="Entity Intelligence · CourtListener" title="Employee Injury & Legal Intelligence" subtitle="Automatically triage a selected Entity’s public court references for employee, injury, workers’ comp, occupational-health, and tort signals instead of dumping a free-text result list." notice="CourtListener results are public research references, not legal conclusions. Entity identity, party role, injury relevance, and case type tags below are heuristic triage aids and require human verification from the underlying docket or opinion.">
    ...
    {data ? <>
      ...
      <div className="grid gap-4 xl:grid-cols-2">{filtered.map(({ reference, classification }) => <Surface key={reference.sourceUrl}><div className="flex items-start justify-between gap-4"><div><p className="text-[9px] font-bold uppercase tracking-[0.16em] text-cyan-50/40">{reference.recordType === "recap" ? "Federal docket / RECAP" : "Opinion"} · {date(reference.dateFiled)}</p><h2 className="mt-2 text-base font-black leading-6 text-white">{reference.caseName}</h2><p className="mt-1 text-[10px] text-cyan-50/43">{reference.court || "Court not reported"}{reference.docketNumber ? ` · ${reference.docketNumber}` : ""}</p></div><button type="button" onClick={() => toggleSave(reference)} className={`rounded-xl border p-2 ${isSaved(reference) ? "border-emerald-200/24 bg-emerald-300/[0.08] text-emerald-100" : "border-white/10 text-cyan-50/45"}`} aria-label={isSaved(reference) ? "Remove saved case" : "Save case"}><BookMarked size={16} /></button></div><div className="mt-3 flex flex-wrap gap-2">{classification.employee ? <span className="rounded-full border border-cyan-200/16 px-2.5 py-1 text-[9px] font-bold text-cyan-100">Employee/workplace</span> : null}{classification.injury ? <span className="rounded-full border border-rose-200/16 px-2.5 py-1 text-[9px] font-bold text-rose-100">Employee injury</span> : null}{classification.workersComp ? <span className="rounded-full border border-violet-200/16 px-2.5 py-1 text-[9px] font-bold text-violet-100">Workers’ compensation</span> : null}{classification.occupationalDisease ? <span className="rounded-full border border-emerald-200/16 px-2.5 py-1 text-[9px]">Occupational disease</span> : null}{classification.disability ? <span className="rounded-full border border-blue-200/16 px-2.5 py-1 text-[9px]">Disability / accommodation</span> : null}{classification.medicalExam ? <span className="rounded-full border border-emerald-200/16 px-2.5 py-1 text-[9px]">Medical examination dispute</span> : null}{classification.workplaceSafety ? <span className="rounded-full border border-amber-200/16 px-2.5 py-1 text-[9px]">OSHA / workplace safety</span> : null}{classification.dba ? <span className="rounded-full border border-violet-200/16 px-2.5 py-1 text-[9px]">DBA / overseas contractor</span> : null}{classification.tort ? <span className="rounded-full border border-amber-200/16 px-2.5 py-1 text-[9px]">Negligence / employee-injury tort</span> : null}</div>{reference.snippet ? <p className="mt-4 line-clamp-6 text-xs leading-6 text-cyan-50/56">{reference.snippet}</p> : <p className="mt-4 text-xs text-cyan-50/38">No public text excerpt was available; open the source for review.</p>}<p className="mt-3 rounded-xl border border-white/8 bg-black/15 p-3 text-[10px] leading-5 text-cyan-50/52"><strong className="text-white">Why it may matter to Occu-Med:</strong> This record contains workplace, injury, medical, safety, disability, compensation, or tort language that may inform employer-service risk research. Verify the parties and allegations in the source.</p><div className="mt-4 flex flex-wrap items-center justify-between gap-3"><span className="text-[9px] text-cyan-50/35">{reference.contentSource || reference.documentDescription || "CourtListener metadata"}</span><SourceLink href={reference.sourceUrl}>Open record</SourceLink></div></Surface>)}{filtered.length === 0 ? <Surface className="xl:col-span-2"><p className="text-sm text-cyan-50/52">No occupational, employment, injury, disability, safety, compensation, or related tort references passed triage.</p></Surface> : null}</div>
