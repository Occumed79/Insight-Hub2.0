...
export default function WarCostsTools() {
  ...
  return <main className="aurora-bg min-h-screen text-white"><Sidebar /><section className="relative z-10 px-5 py-8 lg:ml-[210px] lg:px-10"><div className="flex flex-wrap items-start justify-between gap-4"><HeaderBar eyebrow="WarCosts Intelligence" title="Interactive Tools" subtitle="WarCosts comparisons, rankings, live counters and calculators rebuilt natively inside Insight Hub. The War Map is a separate standalone ArcGIS workspace." /><button onClick={() => void load(true)} disabled={refreshing} className="inline-flex min-h-11 items-center gap-2 rounded-xl border border-cyan-200/20 bg-cyan-300/10 px-4 text-xs font-bold"><RefreshCw size={14} className={refreshing ? "animate-spin" : ""} />Refresh tool data</button></div><WarCostsWorkspaceNav />{error && <GlassCard className="mt-5 border-rose-300/20 p-4 text-sm text-rose-100">{error}</GlassCard>}{loading ? <GlassCard className="mt-5 grid min-h-[420px] place-items-center"><div className="text-center"><Loader2 className="mx-auto animate-spin text-cyan-200" /><p className="mt-3 text-sm font-bold">Loading WarCosts feature data…</p></div></GlassCard> : <div className="mt-5 space-y-5"><div className="grid gap-5 xl:grid-cols-[1fr_1.5fr]"><WarClock annualBudget={annualBudget} /><GlassCard className="p-5"><div className="flex items-center gap-2"><CheckCircle2 size={16} className="text-emerald-300/70" /><p className="text-xs font-black">Native capability layer</p></div><p className="mt-3 text-xs leading-5 text-cyan-100/42">Data Explorer, Compare Wars, timeline, contractor and weapons rankings, president/state/country rankings, Pentagon audit and deployment trackers, tax receipt, jobs simulator, opportunity cost, inflation, lifetime cost, state impact, casualty scaling, draft simulator, war quiz, and Iran live tracking. Operational geography lives in the dedicated War Map workspace.</p></GlassCard></div><section className="grid gap-3 md:grid-cols-2 xl:grid-cols-6">{TOOLS.map((tool) => { const Icon = tool.icon; return <button key={tool.key} onClick={() => setActive(tool.key)} className={`rounded-2xl border p-4 text-left transition ${active === tool.key ? "border-cyan-200/28 bg-cyan-300/12" : "border-white/8 bg-black/10 hover:bg-white/[.035]"}`}><Icon size={18} className={active === tool.key ? "text-cyan-100" : "text-cyan-100/40"} /><p className="mt-3 text-xs font-black">{tool.label}</p><p className="mt-1 text-[9px] leading-4 text-cyan-100/35">{tool.note}</p></button>; })}</section>{active === "explorer" && <DataExplorer conflicts={conflicts} />}{active === "compare" && <CompareWars conflicts={conflicts} />}{active === "timeline" && <Timeline conflicts={conflicts} operations={operations} votes={votes} />}{active === "rankings" && <Rankings data={data} />}{active === "calculators" && <WarCostsCalculators datasets={data} />}{active === "iran" && <IranLive conflicts={conflicts} strikes={strikes} stats={stats} />}</div>}</section></main>;
}
