...
const TOOLS: Array<{ key: ToolKey; label: string; note: string; icon: typeof Search }> = [
  ...
  { key: "compare", label: "Compare Wars", note: "Metrics + interactive bars", icon: GitCompareArrows },
...
function conflictDays(row: WarCostsRow) {
  const start = wcConflictStartTimestamp(row);
  const endYear = wcNumber(row, "endYear");
  const end = endYear ? Date.UTC(endYear, 11, 31) : Date.now();
