...
function PersonalWarCost({ spending, stateRows }: { spending: WarCostsRow[]; stateRows: WarCostsRow[] }) {
  ...
  const perCapita = rows.reduce((sum, row) => { const amount = annualAdjustedSpending(row); const population = wcNumber(row, "population", "usPopulation") || 335_000_000; return sum + (population > 0 ? amount / population : 0); }, 0);
...
function InflationCalculator({ conflicts, spending }: { conflicts: WarCostsRow[]; spending: WarCostsRow[] }) {
  ...
  return <div className="grid gap-5 xl:grid-cols-2"><GlassCard className="p-5"><h3 className="text-lg font-black">War Inflation Comparison</h3><select value={warId} onChange={(e) => setWarId(e.target.value)} className="mt-4 min-h-11 w-full rounded-xl border border-white/10 bg-[#07101c] px-3">{candidates.map((row) => <option key={wcConflictId(row)} value={wcConflictId(row)}>{wcConflictName(row)}</option>)}</select><div className="mt-4 grid grid-cols-2 gap-3"><Metric label="Original cost" value={wcMoney(wcNumber(selected, "costNominal"))} note="source nominal dollars" /><Metric label="2026 / adjusted cost" value={wcMoney(wcConflictCost(selected))} note="source adjusted figure" /></div><div className="mt-3 grid grid-cols-2 gap-3"><Metric label="Per capita then" value={wcNumber(selected, "perCapitaThen", "perCapita") ? wcMoney(wcNumber(selected, "perCapitaThen", "perCapita")) : "—"} note="when source exposes it" /><Metric label="% GDP then" value={wcNumber(selected, "gdpPercent", "percentGdp") ? `${wcNumber(selected, "gdpPercent", "percentGdp")}%` : "—"} note="war-era GDP context" /></div></GlassCard><GlassCard className="p-5"><h3 className="text-lg font-black">Amount + Year Calculator</h3><div className="mt-4 grid grid-cols-2 gap-3"><input type="number" value={amount} onChange={(e) => setAmount(Number(e.target.value))} className="min-h-11 rounded-xl border border-white/10 bg-black/20 px-3" /><select value={year} onChange={(e) => setYear(Number(e.target.value))} className="min-h-11 rounded-xl border border-white/10 bg-[#07101c] px-3">{years.map((value) => <option key={value} value={value}>{value}</option>)}</select></div><p className="mt-5 text-3xl font-black">{multiplier > 0 ? wcMoney(amount * multiplier) : "Source multiplier unavailable"}</p><p className="mt-1 text-xs text-cyan-100/40">{multiplier > 0 ? `${wcMoney(amount)} in ${year} × ${multiplier.toFixed(2)}` : "No nominal/adjusted pair is exposed for this year."}</p></GlassCard></div>;
}
...
function DraftSimulator({ draftSource }: { draftSource: unknown }) {
  ...
  if (income === "Lower") score += 18; if (income === "Higher") score -= 12; if (education === "College") score -= 7; if (education === "Graduate") score -= 12; if (dependents >= 2) score -= 10; if (gender !== "Male") score *= .55;
...
function buildQuiz(conflicts: WarCostsRow[], presidents: WarCostsRow[]): QuizQuestion[] {
  ...
  for (const row of conflicts.filter((item) => wcConflictDeaths(item) > 0).slice(0, 4)) {
    const correct = wcConflictName(row); const others = conflicts.filter((item) => wcConflictId(item) !== wcConflictId(row) && wcConflictDeaths(item) > 0).slice(0, 3).map(wcConflictName);
  ...
  for (const row of presidents.slice(0, 4)) {
    const name = wcText(row, "name", "president", "fullName"); const cost = wcNumber(row, "warCostAdjusted2024", "totalCost"); if (!name || !cost) continue;
...
function AdvancedQuiz({ conflicts, presidents }: { conflicts: WarCostsRow[]; presidents: WarCostsRow[] }) {
  ...
  useEffect(() => { if (finished || answer || !question) return; const timer = window.setInterval(() => setSeconds((value) => { if (value <= 1) { window.clearInterval(timer); window.setTimeout(advance, 0); return 0; } return value - 1; }), 1000); return () => window.clearInterval(timer); }, [answer, finished, index, question]);
...
export default function WarCostsSpecialTools() {
  ...
  const tools: Array<{ key: Tool; label: string; note: string; icon: typeof Calculator }> = [
    { key:"budget", label:"Budget Simulator", note:"Nine category sliders", icon:SlidersHorizontal }, { key:"personal", label:"Personal War Cost", note:"Birth year + state", icon:Calculator },
    { key:"inflation", label:"Inflation", note:"War + amount/year", icon:Calculator }, { key:"draft", label:"Draft Simulator", note:"Full six-field profile", icon:Activity },
    { key:"quiz", label:"Advanced Quiz", note:"20 questions · 15 seconds", icon:Trophy }, { key:"aid", label:"Aid Tax", note:"Tax share by recipient", icon:Calculator },
    { key:"countries", label:"Compare Countries", note:"Military-spending face-off", icon:GitCompareArrows }, { key:"hormuz", label:"Hormuz Impact", note:"Oil, GDP, gas & freight", icon:ShipWheel },
