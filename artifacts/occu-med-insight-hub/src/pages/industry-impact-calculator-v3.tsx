...
function fixed(value: number, digits = 1) { return (Number.isFinite(value) ? value : 0).toLocaleString("en-US", { minimumFractionDigits: digits, maximumFractionDigits: digits }); }

function Kind({ children }: { children: EvidenceKind }) { return <span className="text-[9px] font-semibold uppercase tracking-[.11em] text-slate-500">{children}</span>; }
...
function SelectInput({ label, value, onChange, children, hint }: { label: string; value: string; onChange: (value: string) => void; children: ReactNode; hint?: string }) {
  return <label className="block"><span className="text-[10px] font-medium text-slate-400">{label}</span><select aria-label={label} value={value} onChange={(event) => onChange(event.target.value)} className="mt-1.5 h-10 w-full border border-white/10 bg-[#03080d] px-3 text-[11px] text-white outline-none">{children}</select>{hint ? <p className="mt-1 text-[9px] leading-4 text-slate-600">{hint}</p> : null}</label>;
}

function Metric({ label, value, note, kind }: { label: string; value: string; note: string; kind?: EvidenceKind }) {
  return <div className="border-b border-r border-white/8 px-4 py-4"><div><div className="flex flex-wrap items-center gap-2"><p className="text-[9px] font-semibold uppercase tracking-[.11em] text-slate-600">{label}</p>{kind ? <Kind>{kind}</Kind> : null}</div><p className="mt-1.5 text-2xl font-semibold tracking-[-.03em] text-white">{value}</p></div><p className="mt-1 text-[9px] leading-4 text-slate-500">{note}</p></div>;
}
...
export default function IndustryImpactCalculatorV3() {
  ...
  function selectPrepared(sector: Sector) {
    setSelectedNaics(sector.naics); setBenchmark(sector.benchmark); setMessage(sector.message || "");
  ...
  async function lookup() {
    ...
    try {
      const params = new URLSearchParams({ naics: selectedNaics.trim() }); if (year.trim()) params.set("year", year.trim());
      const response = await fetch(`/api/bls/industry-benchmark?${params}`); const payload = await response.json();
      if (!response.ok || !payload.ok) throw new Error(payload.error || "BLS lookup failed.");
      setBenchmark(payload.benchmark ?? null); setMessage(payload.message || payload.limitation || "");
      if (payload.benchmark?.trcRate != null) setTargetTrir(payload.benchmark.trcRate);
    } catch (reason) { setMessage(reason instanceof Error ? reason.message : "BLS lookup failed."); }
    finally { setLoading(false); }
  }

  function clearSample() {
    setWorkforce(0); setHoursPerWorker(0); setObservedTrir(0); setObservedDart(0); setTargetTrir(0); setLostDaysPerRecordable(0); setLowCost(0); setBaseCost(0); setHighCost(0); setIndirectMultiplier(0); setProfitMargin(0); setBenchmark(null); setSelectedNaics(""); setYear(""); setMessage("");
  }

  const model = useMemo(() => {
    const workers = Math.max(workforce, 0); const hoursEach = Math.max(hoursPerWorker, 0); const modeledAnnualHours = workers * hoursEach;
    const currentRate = Math.max(observedTrir, 0); const currentDartRate = Math.max(observedDart, 0); const benchmarkRate = Math.max(benchmark?.trcRate ?? 0, 0); const benchmarkDartRate = Math.max(benchmark?.dartRate ?? 0, 0); const targetRate = Math.max(targetTrir > 0 ? targetTrir : benchmarkRate, 0);
    const casesAt = (rate: number) => modeledAnnualHours > 0 ? rate * modeledAnnualHours / 200_000 : 0;
    const currentCases = casesAt(currentRate); const currentDartCases = casesAt(currentDartRate); const benchmarkCases = casesAt(benchmarkRate); const benchmarkDartCases = casesAt(benchmarkDartRate); const targetCases = casesAt(targetRate); const avoidedCases = Math.max(currentCases - targetCases, 0); const excessVsBenchmark = Math.max(currentCases - benchmarkCases, 0); const gapPercent = benchmarkRate > 0 ? ((currentRate / benchmarkRate) - 1) * 100 : null;
    const lostDaysCurrent = currentCases * Math.max(lostDaysPerRecordable, 0); const lostDaysTarget = targetCases * Math.max(lostDaysPerRecordable, 0); const avoidedLostDays = Math.max(lostDaysCurrent - lostDaysTarget, 0);
    ...
    const margin = Math.max(profitMargin, 0) / 100; const salesRecovery = margin > 0 ? savings.base / margin : 0;
    const trajectory = Array.from({ length: 6 }, (_, index) => { const fraction = index / 5; const rate = currentRate + (targetRate - currentRate) * fraction; const cases = casesAt(rate); return { year: index === 0 ? "Now" : `Year ${index}`, rate, cases, lostDays: cases * Math.max(lostDaysPerRecordable, 0), cost: cases * Math.max(baseCost, 0) * multiplier }; });
    return { modeledAnnualHours, currentRate, currentDartRate, benchmarkRate, benchmarkDartRate, targetRate, currentCases, currentDartCases, benchmarkCases, benchmarkDartCases, targetCases, avoidedCases, excessVsBenchmark, gapPercent, lostDaysCurrent, lostDaysTarget, avoidedLostDays, currentCost, targetCost, savings, salesRecovery, trajectory };
    ...
    <section className="min-h-screen lg:ml-[210px]">
      ...
      <div className="grid min-h-[calc(100vh-150px)] 2xl:grid-cols-[360px_minmax(0,1fr)]">
        <aside className="border-r border-white/8 bg-[#050a10]/94">
          ...
          <section className="border-b border-white/8 p-5"><Kind>Official benchmark</Kind><h2 className="mt-1 text-base font-semibold text-white">Prepared BLS industry library</h2><p className="mt-1 text-[9px] leading-4 text-slate-600">Select by industry name. NAICS remains visible only for traceability.</p>{!overview ? <div className="mt-4 flex items-center gap-2 text-[10px] text-slate-500"><Loader2 size={13} className="animate-spin" />Loading BLS…</div> : <div className="mt-3 max-h-48 overflow-y-auto border-y border-white/8">{overview.sectors.map((sector) => <button key={sector.id} type="button" onClick={() => selectPrepared(sector)} className={`flex w-full items-center justify-between gap-3 border-b border-white/[.055] px-1 py-2.5 text-left last:border-b-0 ${selectedNaics === sector.naics && benchmark ? "text-white" : "text-slate-500 hover:text-slate-300"}`}><span className="text-[10px] font-semibold">{sector.label}</span><span className="text-[9px]">{sector.benchmark?.trcRate != null ? `${number(sector.benchmark.trcRate, 1)} TRC` : "—"}</span></button>)}</div>}
        ...
        <div className="min-w-0 p-5 xl:p-7">
          <section className="border border-white/8 bg-[#060c13]/78">
            <div className="flex flex-wrap items-start justify-between gap-4 border-b border-white/8 px-5 py-5"><div><Kind>Modeled output</Kind><h2 className="mt-1 text-2xl font-semibold tracking-[-.025em] text-white">Live employer impact</h2><p className="mt-2 max-w-3xl text-[10px] leading-5 text-slate-500">Changing workforce size or hours immediately rescales cases, lost workdays and dollars. No output below is presented as a forecast.</p></div><Gauge size={20} className="text-violet-300/50" /></div>
            <div className="grid sm:grid-cols-2 xl:grid-cols-4"><Metric label="Affected workers / recordables" value={ready ? number(model.currentCases, 1) : "—"} note={`${number(workforce, 0)} ${workforceBasis === "fte" ? "FTE" : "workers"} × ${number(model.currentRate, 2)} TRIR`} kind="Modeled output" /><Metric label="Target recordables" value={ready ? fixed(model.targetCases, 1) : "—"} note={`${number(model.avoidedCases, 1)} modeled cases avoided`} kind="Modeled output" /><Metric label="Current lost workdays" value={ready && lostDaysPerRecordable > 0 ? number(model.lostDaysCurrent, 0) : "—"} note={`${number(model.avoidedLostDays, 0)} modeled days avoided`} kind="Modeled output" /><Metric label="Base annual savings" value={ready && baseCost > 0 ? money(model.savings.base) : "—"} note="Current scaled cost − target cost" kind="Modeled output" /></div>
          </section>
