  ...
  return <main className="aurora-bg reviewer-native-page min-h-screen pb-24 text-white"><Sidebar/><section className="relative z-10 px-5 py-8 pt-24 lg:ml-[210px] lg:px-12 lg:pt-8"><HeaderBar eyebrow="Standards / Interaction Engine" title="Standards Intelligence" subtitle="Choose the controlling frameworks, enter one scenario, then read the primary determination before drilling into supporting rules and citations."/>
    <div className="rh-stack">
      ...
      <section className="rh-support-grid">
        <div className="rh-card is-full"><div className="flex items-center justify-between gap-4"><div><div className="rh-label">04 · Supporting matched rules</div><h3 className="mt-2">{findings.length} findings across {frameworks.length} framework{frameworks.length===1?"":"s"}</h3></div><span className="rounded-full border border-cyan-100/14 px-3 py-1 text-[9px] font-black uppercase tracking-[.13em] text-cyan-100/60">Full reviewer engine</span></div><div className="mt-5 grid gap-3 xl:grid-cols-2">{findings.map((finding)=>{const source=STANDARD_SOURCES[finding.standardId];return <article key={`${finding.standardId}-${finding.id}`} className="rounded-2xl border border-white/10 bg-white/[.022] p-4"><div className="flex flex-wrap items-center gap-2"><span className={`rounded-full border px-2.5 py-1 text-[9px] font-black uppercase tracking-[.14em] ${tone[finding.level]}`}>{finding.level}</span><span className="rounded-full border border-white/10 px-2.5 py-1 text-[9px] font-bold uppercase tracking-[.12em] text-cyan-50/55">{source.shortLabel}</span>{finding.topics.slice(0,2).map((topic)=><span key={topic} className="text-[9px] text-violet-100/42">{topic}</span>)}</div><h4 className="mt-3 text-sm font-black">{finding.title}</h4><p className="mt-2 text-xs leading-5 text-cyan-100/54">{finding.summary}</p><div className="mt-3 rounded-xl border border-cyan-100/8 bg-cyan-300/[.025] p-3 text-xs leading-5 text-cyan-50/62"><strong>Reviewer action:</strong> {finding.action}</div><div className="mt-3 flex flex-wrap items-center justify-between gap-2"><span className="text-[10px] text-cyan-100/36">{finding.citation}</span><a href={finding.sourceUrl} target="_blank" rel="noreferrer" className="inline-flex items-center gap-1 text-[10px] font-black text-cyan-100/64 hover:text-white">Source <ExternalLink size={10}/></a></div></article>})}</div></div>
