...
export default function DataProfiles() {
  ...
  return (
    <main className="aurora-bg min-h-screen text-white"><Sidebar /><section className="relative z-10 px-5 py-8 lg:ml-[210px] lg:px-12">
      ...
      <div className="mt-5 space-y-4">{profile.sections.map((section, index) => (<SectionPanel key={section.id} title={section.title} narrative={section.narrative} defaultOpen={index < 2}><div className="grid gap-4 md:grid-cols-[1fr_.8fr]"><ul className="space-y-2 text-sm text-cyan-50/72">{section.bullets.map((bullet) => <li key={bullet} className="rounded-2xl border border-cyan-100/10 bg-white/[0.03] px-4 py-3">{bullet}</li>)}</ul><div className="grid gap-3">{section.metrics.map((id) => dataset.metrics.find((metric) => metric.id === id)).filter(Boolean).map((metric) => metric ? <MetricCard key={metric.id} metric={metric} /> : null)}</div></div></SectionPanel>))}</div>
