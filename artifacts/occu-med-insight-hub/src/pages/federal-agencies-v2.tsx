...
const NEGATED_RECOMPETE_EVIDENCE = /\b(?:no|not|without)\s+(?:contract\s+)?(?:recompete|re-compete|renewal|expiration|follow[- ]?on|option\s+year|bridge\s+contract)(?:\s+(?:or|and)\s+(?:recompete|re-compete|renewal|expiration|follow[- ]?on|option\s+year|bridge\s+contract))*\b/i;
const VIEWS = [
  "Overview",
  "Solicitations",
  "Contracts & Spending",
...
export default function FederalAgenciesV2Page() {
  ...
  const hasTypeEvidence = (item: PersistedItem, type: "recompete" | "forecast") => {
    ...
    if (type === "recompete" && NEGATED_RECOMPETE_EVIDENCE.test(evidence)) return false;
    return (type === "recompete" ? RECOMPETE_EVIDENCE : FORECAST_EVIDENCE).test(evidence);
  };
