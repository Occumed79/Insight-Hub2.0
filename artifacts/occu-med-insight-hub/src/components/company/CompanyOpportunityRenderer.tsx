...
import { finiteChartNumber } from "../../data/visualizationValidity";
...
export function CompanyOpportunityRenderer({ data, companyName }: { data: OpportunityMatrixPoint[]; companyName: string }) {
  const validData = data.filter((point) => finiteChartNumber(point.revenuePotential) !== undefined);
    ...
    <div className="mt-5">
      <ChartBlock title={`${companyName} opportunity matrix`} subtitle="Revenue potential vs implementation complexity with strategic value bubble size.">
        <ScatterChart>
          ...
          <XAxis dataKey="revenuePotential" name="Revenue potential (source units)" stroke="rgba(207,250,254,.45)" tick={{ fontSize: 10 }} />
          ...
          <Scatter name="Opportunity" data={validData} fill="#22c55e" />
        </ScatterChart>
      </ChartBlock>
    </div>
  );
