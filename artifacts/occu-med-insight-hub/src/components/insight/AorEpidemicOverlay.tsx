...
type YellowBookSourceAsset = { type: "table" | "map" | "figure" | "box"; id: string; page: number; title: string; excerpt: string };
type YellowBookProfile = { title: string; aliases: string[]; pages: [number, number]; sourceDate: string; agent: string; endemicity: string; atRisk: string; prevention: string; diagnosticSupport: string; transmission: string; clinical: string; diagnosis: string; treatment: string; keyNotes: string[]; operationalRules?: string[]; sourceAssets?: YellowBookSourceAsset[]; flags: Record<string, boolean> };
type YellowBookPayload = { ok: boolean; source?: { publication?: string; edition?: number; bookletPages?: number; diseaseChapters?: number; structuredAssets?: number; currentGuidanceBoundary?: string }; profiles: YellowBookProfile[]; error?: string };
...
export function AorEpidemicOverlay({ map, mapStatus, selectedCountry, travelHealth }: Props) {
  ...
  const yellowBookNotes = selectedYellowBook ? [...new Set([...(selectedYellowBook.operationalRules || []), ...(selectedYellowBook.keyNotes || [])])] : [];
    ...
    {(mode !== "current" || selectedCountry || currentTrackers.length > 0) ? <div className={`absolute right-3 top-[102px] z-10 rounded-2xl border border-white/12 bg-[#020812]/90 p-3 shadow-[0_18px_50px_rgba(0,0,0,.34)] backdrop-blur-2xl ${(mode === "yellowbook" || mode === "destination" || mode === "notices") ? "max-h-[560px] w-[min(92%,430px)] overflow-y-auto" : "w-[min(88%,310px)]"}`}>
      {mode === "current" ? <>
        ...
        {yellowBookError ? <p className="mt-2 text-[9px] leading-4 text-amber-100/68"><AlertTriangle size={10} className="mr-1 inline" />{yellowBookError}</p> : yellowBook && selectedYellowBook ? <div className="mt-2 space-y-2.5">
          ...
          {yellowBookNotes.length ? <details open className="rounded-xl border border-amber-200/10 bg-amber-300/[0.025] p-2.5"><summary className="cursor-pointer text-[8px] font-black uppercase tracking-[0.12em] text-amber-100/48"><span className="inline-flex items-center gap-1"><Syringe size={10} />Operational notes · {yellowBookNotes.length}</span></summary><div className="mt-2 space-y-1.5">{yellowBookNotes.map((note, index) => <p key={`${index}-${note.slice(0, 28)}`} className="text-[8px] leading-3.5 text-amber-50/58">• {note}</p>)}</div></details> : null}
          {selectedYellowBook.sourceAssets?.length ? <details className="rounded-xl border border-cyan-100/10 bg-cyan-300/[0.025] p-2.5"><summary className="cursor-pointer text-[8px] font-black uppercase tracking-[0.12em] text-cyan-100/50">Source tables, maps, figures & boxes · {selectedYellowBook.sourceAssets.length}</summary><div className="mt-2 space-y-2">{selectedYellowBook.sourceAssets.map((asset) => <div key={`${asset.type}-${asset.id}-${asset.page}`} className="rounded-lg border border-white/8 bg-white/[0.02] p-2"><div className="flex items-start justify-between gap-2"><strong className="text-[8px] leading-3 text-white/68">{asset.title}</strong><span className="shrink-0 rounded-full border border-white/9 px-1.5 py-0.5 text-[7px] uppercase text-cyan-100/35">{asset.type} · p. {asset.page}</span></div><p className="mt-1 text-[8px] leading-3.5 text-cyan-50/46">{asset.excerpt}</p></div>)}</div></details> : <p className="rounded-xl border border-white/7 bg-white/[0.015] p-2 text-[8px] leading-3.5 text-cyan-100/30">No distinct table, map, figure or box was indexed for this chapter in the supplied booklet; the full chapter profile and operational rules remain available above.</p>}
        ...
        <p className="mt-2 text-[8px] leading-3 text-cyan-100/30">Historical dataset includes 1996–March 2022 occurrence records. The paper’s published ESDA/LISA analysis excludes 2022 because only its first three months were available and therefore uses 1996–2021.</p>
      </>}
    </div> : null}
