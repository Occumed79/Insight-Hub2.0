...
export function OccupationalToolShell({ eyebrow, title, subtitle, notice, children }: { eyebrow: string; title: string; subtitle: string; notice: string; children: ReactNode }) {
  return <main className="aurora-bg min-h-screen pb-20 text-white"><Sidebar /><section className="relative z-10 px-5 py-7 lg:ml-[210px] lg:px-10 2xl:px-14"><HeaderBar eyebrow={eyebrow} title={title} subtitle={subtitle} /><div className="mb-6 border-y border-slate-300/10 py-3"><div className="flex items-start gap-3 text-[14px] leading-6 text-slate-300"><ShieldCheck size={16} className="mt-1 shrink-0 text-cyan-200/72" /><p>{notice}</p></div></div>{children}</section></main>;
}
...
export function SectionTabs<T extends string>({ tabs, active, onChange }: { tabs: Array<{ id: T; label: string; icon?: IconComponent }>; active: T; onChange: (id: any) => void }) {
  return <div className="mb-7 flex gap-6 overflow-x-auto border-b border-slate-300/10">{tabs.map((tab) => { const Icon = tab.icon; return <button key={tab.id} type="button" onClick={() => onChange(tab.id)} className={cn("inline-flex min-h-11 shrink-0 items-center gap-2 border-b-2 px-1 text-[14px] font-bold transition", active === tab.id ? "border-cyan-200/80 text-white" : "border-transparent text-slate-300/70 hover:text-white")}>{Icon ? <Icon size={15} /> : null}{tab.label}</button>; })}</div>;
}
...
export function MetricOrb({ label, value, note, icon: Icon = Sparkles, tone = "cyan" }: { label: string; value: string; note: string; icon?: IconComponent; tone?: "cyan" | "violet" | "emerald" | "rose" | "amber" }) {
  const toneClasses = { cyan: "text-cyan-200", violet: "text-violet-200", emerald: "text-emerald-200", rose: "text-rose-200", amber: "text-amber-200" }[tone];
  return <motion.div layout initial={{ opacity: 0, scale: 0.98 }} animate={{ opacity: 1, scale: 1 }} className="min-w-0 border-l border-slate-300/10 py-2 pl-4 pr-2 first:border-l-0 first:pl-0"><div className="flex items-center justify-between gap-3"><p className="text-[12px] font-bold uppercase tracking-[0.12em] text-slate-300/76">{label}</p><Icon size={15} className={cn("opacity-75", toneClasses)} /></div><p className="mt-2 break-words text-2xl font-black tracking-[-0.035em] text-white">{value}</p><p className="mt-1 text-[13px] leading-5 text-slate-300/82">{note}</p></motion.div>;
}
