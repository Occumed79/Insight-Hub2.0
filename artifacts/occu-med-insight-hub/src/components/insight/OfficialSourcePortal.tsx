...
const OCCUPATIONAL_SOURCES: SourcePortal[] = [
  {
    ...
    frameTitle: "BLS Injuries, Illnesses & Fatalities official data portal",
  },
  {
    id: "osha",
    ...
    frameTitle: "OSHA Data official data portal",
  },
  {
    id: "datagov",
    ...
    frameTitle: "Data.gov Catalog official data portal",
  },
];
...
const ONET_SOURCE: SourcePortal = {
  ...
  frameTitle: "O*NET OnLine official data portal",
};
  ...
  return (
    <section className="relative overflow-hidden rounded-[30px] border border-cyan-100/24 bg-[#010611] shadow-[0_32px_100px_rgba(0,0,0,.55),0_0_48px_rgba(34,211,238,.08),inset_0_1px_0_rgba(255,255,255,.08)]">
      ...
      <div className="relative min-h-[720px] bg-white">
        <iframe key={`${source.id}-${frameKey}`} src={proxyUrl} title={source.frameTitle} className="block h-[78vh] min-h-[720px] w-full bg-white" loading="eager" referrerPolicy="no-referrer" sandbox="allow-forms allow-modals allow-popups allow-popups-to-escape-sandbox allow-scripts allow-downloads" />
      </div>
    </section>
  );
}
...
export function OfficialSourcePortal({ mode }: { mode: OfficialSourcePortalMode }) {
  ...
  return (
    <div className="mb-7">
      <span className="sr-only">Official source portal</span>
      {mode === "onet" ? <span className="sr-only">O*NET OnLine</span> : null}
      <div className="mb-4 flex flex-wrap items-end justify-between gap-4">
        <div><p className="text-[10px] font-black uppercase tracking-[0.18em] text-cyan-100/52">Immersive official workspace</p><h2 className="mt-1 text-xl font-black tracking-[-0.025em] text-white">{active.label}</h2><p className="mt-1 max-w-4xl text-xs leading-5 text-cyan-50/52">{active.description}</p></div>
        {sources.length > 1 ? <div className="flex flex-wrap gap-2" role="tablist" aria-label="Official occupational data source">{sources.map((source) => <button key={source.id} type="button" role="tab" aria-selected={active.id === source.id} onClick={() => setActiveId(source.id)} className={`min-h-10 rounded-xl border px-4 text-xs font-black transition ${active.id === source.id ? "border-cyan-200/30 bg-cyan-300/[0.12] text-white shadow-[0_0_24px_rgba(34,211,238,.08)]" : "border-white/10 bg-[#071321]/76 text-cyan-50/58 hover:border-cyan-200/18 hover:text-white"}`}>{source.shortLabel}</button>)}</div> : null}
      </div>
