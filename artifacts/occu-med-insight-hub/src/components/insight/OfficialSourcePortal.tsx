  ...
  return (
    <div className="mb-7">
      ...
      <div className="mb-4 flex flex-wrap items-end justify-between gap-4">
        ...
        {sources.length > 1 ? <div className="flex flex-wrap gap-2" aria-label="Official occupational data source">{sources.map((source) => <button key={source.id} type="button" aria-pressed={active.id === source.id} onClick={() => setActiveId(source.id)} className={`min-h-10 rounded-xl border px-4 text-xs font-black transition ${active.id === source.id ? "border-cyan-200/30 bg-cyan-300/[0.12] text-white shadow-[0_0_24px_rgba(34,211,238,.08)]" : "border-white/10 bg-[#071321]/76 text-cyan-50/58 hover:border-cyan-200/18 hover:text-white"}`}>{source.shortLabel}</button>)}</div> : null}
      </div>
